	...
	public static void main(String[] args)
	{
		...
		while (run)
		{
			try 
			{
				run2 = true;
				System.out.println("----------------------------------------------------------------------------------------------------");
				System.out.println("1. 상품정보 | 2. 사용자정보 | 3. 주문정보 | 4. 주문완료처리 | 5. 주문조회(일자) | 6.월별 매출금액 조회 | 7. 미완료 메일발송");
				System.out.println("----------------------------------------------------------------------------------------------------");
				System.out.print("선택>");
									...
									else
									{
										...
										for (int i = 0 ; i < items.size() ; i++)
										{
											System.out.printf("\t%s\t||\t%s\t||\t%d\t||\t%s\t\n", items.get(i)
													.getItem_code(), items.get(i).getItem_name(), items.get(i).getPrice(),
													items.get(i).getSpec());
										}
										break;
									}
									...
									else
									{
										...
										for (int i = 0 ; i < orders.size() ; i++)
										{
											System.out.printf("\t%s\t\t||\t%s\t\t||\t%d\t\t||\t%s\t||\t%s\t\n", orders.get(
													i).getUser_name(), orders.get(i).getItem_name(), orders.get(i)
															.getItem_qty(), orders.get(i).getPhone(), orders.get(i)
																	.getAddress());
										}
										break;
									}
					...
					case 5 :
						...
						while (true)
						{
							...
							else
							{
								...
								{
									System.out.printf("\t%s\t\t||\t%s\t\t||\t%d\t\t||\t%s\t||\t%s\t\n", orders.get(i)
											.getUser_name(), orders.get(i).getItem_name(), orders.get(i).getItem_qty(),
											orders.get(i).getPhone(), orders.get(i).getAddress());
								}
								break;
							}
						}
						...
						break;
					case 6 : 
						System.out.print("조회년월 YYYYMM>");
						String ordym = sc.nextLine();
						if(!isValidYYYYMM(ordym)) 
						{
							System.out.println("날짜 형식이 잘못되었습니다.");
						}

						ArrayList<Orders> orders = ordersdao.getOrderPrice(ordym);

						if (orders.size() < 1)
						{
							System.out.println("매출금액조회 실패.");
						}
						else
						{
							System.out.println("\t년월\t||\t\t매출금액");

							for (int i = 0 ; i < orders.size() ; i++)
							{
								System.out.printf("\t%s\t||%s\t\n", orders.get(i).getOrder_ym(), orders.get(i).getSum_price());
							}
						}
						break;
					case 7 :
			...
			{
				System.out.println("빈칸은 입력할 수 없습니다.");
			}
		}
		// commit test
	}
	
    public static boolean isValidYYYYMM(String input) 
    {
